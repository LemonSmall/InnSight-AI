...
public interface CreditLedgerMapper extends BaseMapper<CreditLedger> {
    ...
    @Select("SELECT * FROM credit_ledger WHERE tenant_id = #{tenantId} ORDER BY created_at DESC LIMIT #{limit}")
    List<CreditLedger> findByTenant(Long tenantId, int limit);
    ...
    int todayConsumed(Long tenantId);
}
