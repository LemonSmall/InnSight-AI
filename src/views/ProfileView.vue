...
import {
  ...
  Shield, Phone, Mail, Loader2, Copy
...
function saveInfo() {
  auth.updateProfile({ name: editName.value, phone: editPhone.value })
...
const newEmp = ref({ name: '', phone: '', role: 'front_desk' as Role })
const empError = ref('')
...
function addEmployee() {
  ...
  auth.addEmployee({ name: newEmp.value.name, phone: newEmp.value.phone, role: newEmp.value.role, avatar: '' })
  newEmp.value = { name: '', phone: '', role: 'front_desk' }
...
function saveEditEmp() {
  ...
  auth.updateEmployee(editingEmpId.value, { ...editEmp.value })
  ...
  <div class="h-full flex flex-col max-w-3xl mx-auto w-full">
    ...
    <div v-if="activeTab === 'info'" class="bg-white border border-cream-200 rounded-xl p-5 space-y-4">
      ...
      <div>
        ...
        <input v-model="editName" :disabled="!infoEditing" :class="['w-full text-sm px-3 py-2 rounded-lg border transition-colors', infoEditing ? 'border-bamboo-400 bg-white text-warm-800 focus:outline-none' : 'border-cream-200 bg-cream-50 text-warm-600']" />
      </div>
      <div>
        <label class="text-xs font-medium text-warm-700 mb-1 block">手机号</label>
        <input v-model="editPhone" :disabled="!infoEditing" :class="['w-full text-sm px-3 py-2 rounded-lg border transition-colors', infoEditing ? 'border-bamboo-400 bg-white text-warm-800 focus:outline-none' : 'border-cream-200 bg-cream-50 text-warm-600']" />
      </div>
      <div class="flex gap-2">
        ...
        <template v-else>
          <button @click="saveInfo" class="text-xs px-4 py-2 rounded-lg bg-bamboo-800 text-white hover:bg-bamboo-900 transition-colors flex items-center gap-1.5">
    ...
    <div v-if="activeTab === 'password'" class="bg-white border border-cream-200 rounded-xl p-5 space-y-4">
      ...
      <div>
        ...
        <input v-model="oldPwd" type="password" placeholder="请输入旧密码" class="w-full text-sm px-3 py-2 rounded-lg border border-cream-300 bg-white text-warm-800 focus:outline-none focus:border-bamboo-400 transition-colors" />
      </div>
      <div>
        <label class="text-xs font-medium text-warm-700 mb-1 block">新密码</label>
        <input v-model="newPwd" type="password" placeholder="至少6位" class="w-full text-sm px-3 py-2 rounded-lg border border-cream-300 bg-white text-warm-800 focus:outline-none focus:border-bamboo-400 transition-colors" />
      </div>
      <div>
        <label class="text-xs font-medium text-warm-700 mb-1 block">确认新密码</label>
        <input v-model="confirmPwd" type="password" placeholder="再次输入新密码" class="w-full text-sm px-3 py-2 rounded-lg border border-cream-300 bg-white text-warm-800 focus:outline-none focus:border-bamboo-400 transition-colors" />
      </div>
    ...
    <div v-if="activeTab === 'employees' && auth.canManageEmployees()" class="bg-white border border-cream-200 rounded-xl p-5 space-y-4">
      <div class="flex items-center justify-between">
        ...
        <button @click="showAddEmp = !showAddEmp" class="text-xs px-3 py-1.5 rounded-lg bg-bamboo-800 text-white hover:bg-bamboo-900 transition-colors flex items-center gap-1.5">
      ...
      <div v-if="showAddEmp" class="bg-cream-50 border border-cream-200 rounded-lg p-4 space-y-3">
        ...
        <div class="grid grid-cols-3 gap-3">
          <input v-model="newEmp.name" placeholder="姓名" class="text-xs px-3 py-2 rounded-lg border border-cream-300 bg-white text-warm-800 focus:outline-none focus:border-bamboo-400" />
          <input v-model="newEmp.phone" placeholder="手机号" class="text-xs px-3 py-2 rounded-lg border border-cream-300 bg-white text-warm-800 focus:outline-none focus:border-bamboo-400" />
          <select v-model="newEmp.role" class="text-xs px-3 py-2 rounded-lg border border-cream-300 bg-white text-warm-800 focus:outline-none focus:border-bamboo-400">
            <option v-for="(label, key) in roleLabels" :key="key" :value="key" v-show="key !== 'admin'">{{ label }}</option>
          </select>
        </div>
        <div class="flex gap-2">
          <button @click="addEmployee" class="text-xs px-4 py-2 rounded-lg bg-bamboo-800 text-white hover:bg-bamboo-900 transition-colors">确认添加</button>
        ...
        <div v-for="emp in auth.employees" :key="emp.id" class="flex items-center justify-between p-3 border border-cream-200 rounded-lg hover:border-cream-300 transition-colors">
          <template v-if="editingEmpId === emp.id">
            ...
            <div class="flex gap-1.5">
              <button @click="saveEditEmp" class="text-[10px] p-1.5 rounded bg-bamboo-800 text-white hover:bg-bamboo-900"><Check class="w-3 h-3" /></button>
          ...
          <template v-else>
            ...
            <div class="flex items-center gap-3 flex-1 min-w-0">
              ...
              <div class="min-w-0">
                <div class="text-xs font-medium text-warm-800">{{ emp.name }}</div>
            ...
            <div class="flex gap-1.5">
              ...
              <button @click="removeEmployee(emp.id)" class="text-[10px] p-1.5 rounded border border-red-200 text-red-400 hover:bg-red-50"><Trash2 class="w-3 h-3" /></button>
            </div>
          </template>
        </div>
      </div>
